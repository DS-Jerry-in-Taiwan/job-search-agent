...
def test_workflow_e2e_mid_score():
    ...
    app = workflow_registry["default"]
    result = app.invoke(state)
    # 僅允許 decision_result 在 result 頂層
    assert "decision_result" in result
    dr = result["decision_result"]
    assert dr is not None
    assert 0.4 <= dr["final_score"] < 0.8
    assert result.get("is_complete") or result["system"].get("workflow_status") == "completed"
